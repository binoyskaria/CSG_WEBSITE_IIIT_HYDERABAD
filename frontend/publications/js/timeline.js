...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch("http://localhost:3000/api/getPublications")
      ...
      .then(data => {
        // Process the fetched data and dynamically add events to the timeline
        const timeline = document.getElementById("tL");
  
        data.events.forEach(event => {
          const newEvent = document.createElement("div");
          newEvent.className = "tL event";
          newEvent.setAttribute("data-side", event.side);
          newEvent.setAttribute("data-timer", event.timer);
  
          newEvent.innerHTML = `
            <h4 class="tL title">${event.title}</h4>
            <time class="tL date">${event.date}</time>
            <div class="tL body">
              <h3>${event.body}</h3>
            </div>
          `;
  
          timeline.insertBefore(newEvent, timeline.querySelector(".endBall"));
        });
      })
